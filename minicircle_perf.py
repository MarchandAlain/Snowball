# Minimum circle algorithm in 2-D - Alain R. Marchand - February 2023
import os
import math
import numpy as np
import time
...
def circle2(A, B, remain):
    ...
    if not point_outside(remain, 0, O, r_squared)[0]:
        return O, r_squared, True                       # all points are in the circle 
...
def circle3(subset):
    ...
    if (x3-x2)*(y2-y1) == (x2-x1)*(y3-y2) :                                # check for alignment              
        return minimal2(subset)[:2]                                             # print(subset, "are aligned")
...
def minimal2(subset):
    ...
    for i, j, k, l in combine:
        ...
        # calculate circle
        O, r_squared, ok = circle2(A, B, remain)
        if ok:
            return O, r_squared, [A, B], True                       # all points are in the circle 
            
    return None, None, [None, None], False                  # no solution with 2 points           
...
#====================================== minicircle
def minicircle(points, subset, position):
    """
    compute minimum circle enclosing points
    ...
    # search for minimum circle
    while True:
        
        # count steps (new center and radius)
        steps+=1

        # try solving with 2 points
        P, s, [A, B], ok = minimal2(subset)                                  
        if ok:
            O, r_squared=P, s
            subset=[A, B]
        # solve with 3 points            
        else:
            O, r_squared, [A, B, C] = minimal3(subset)                
            subset=[A, B, C]
        ...
        # try adding another outside point
        if len(subset)==3:
            D, position=point_outside(points, position, O, r_squared)
            if D:
                subset+=[D]
...
for index, Set_size in enumerate(Size_list):   
    for rep in range(Repeat):
        ...
        position=0
        subset, O, r_squared=minicircle(points, subset, position)
